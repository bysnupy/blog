...
# まとめ
簡単にstartupProbeがどのように動作するかみてみました。コンテナの稼働監視が起動フェーズと運用フェーズを分けて別途指定できるようになってより適切な監視ができるようになって安定的なサービス監視及び提供に役に立つ構成が設定できるようになったことが確認できました。依存サービスの監視及び起動順序の制御にもstartupProbeが活用できると思いますので皆さんもお試しください。
